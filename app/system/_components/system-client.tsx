...
export function SystemClient() {
  ...
  const toggleSetting = async (field: 'pollingEnabled' | 'webhookEnabled', value: boolean) => {
    const setter = field === 'pollingEnabled' ? setSavingPolling : setSavingWebhook;
    setter(true);
    try {
      const res = await fetch('/api/system', {
        method: 'PATCH',
          ...
          <CardContent className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-white font-medium">Polling Bridge</p>
                  <p className="text-xs text-slate-500">Periodic inbox scan fallback</p>
                </div>
              </div>
              <div className="flex items-center gap-2">
                {savingPolling && <Loader2 className="h-3 w-3 animate-spin text-slate-400" />}
                <Switch
                  checked={data?.pollingEnabled ?? false}
                  onCheckedChange={(v) => toggleSetting('pollingEnabled', v)}
                  disabled={loading || savingPolling}
                />
              </div>
            </div>
          </CardContent>
